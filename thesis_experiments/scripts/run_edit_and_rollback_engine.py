...
def run_edit_and_rollback_engine(
    ...
    if exp_hparams_path_raw:
        ...
        for piece in exp_hparams_path_pieces:
            exp_hparams_path_s += piece.strip()
    ...
    if exp_hparams_path is not None:
        log_step(f"Using hparams file: {exp_hparams_path}")
    else:
        log_step("Using provided hparams object.")
    ...
    model_parallel = bool(getattr(runtime_hparams, "model_parallel", False))
    ...
    # ----------------------------
    # Finalize and return results payload
    # ----------------------------
